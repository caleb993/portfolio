...
# initialize DB (creates database + tables if needed)
try:
    data.init_db()
except Exception as e:
    # initialization error will surface at runtime — still allow app to start
    print("Warning: data.init_db() failed:", e)
...
# Blog storage (no longer file-backed, but keep these paths for compatibility/export)
BLOG_DIR = os.path.join(DATA_DIR, "blog")
BLOG_CSV = os.path.join(BLOG_DIR, "blogs.csv")

# Newsletter subscribers CSV (kept for export compatibility)
...
# ---- Database-backed message functions (wrappers to data.py) ----
def save_message(name: str, email: str, message: str) -> None:
    data.add_message(name.strip(), email.strip(), message.strip())

def load_messages():
    return data.fetch_messages()

def save_all_messages(msgs):
    # replace all messages in DB with provided list
    data.replace_all_messages(msgs)

# ---- Blogs (DB-backed) ----
def load_blogs():
    return data.fetch_blogs()

def save_all_blogs(blogs):
    data.replace_all_blogs(blogs)

def add_blog(title, content):
    # Create slug using existing slugify in this file
    slug = slugify(title) or f"post-{int(datetime.now().timestamp())}"
    return data.add_blog(title.strip(), slug, content.strip())

def delete_blog_by_id(bid):
    data.delete_blog_by_id(bid)

# ---- Newsletter subscribers (DB-backed) ----
def save_subscriber(email: str) -> None:
    data.add_subscriber(email.strip().lower())

def load_subscribers():
    return data.fetch_subscribers()
...
# ====== BLOG HELPERS (wrappers above use data module) ======
...
def toggle_read_json(mid):
    if not require_admin():
        return jsonify({"error": "unauthorized"}), 403
    new = data.toggle_read_by_index(mid)
    if new is None:
        return jsonify({"error":"not found"}), 404
...
def delete_message(mid):
    if not require_admin():
        return redirect(url_for("login"))
    deleted = data.delete_message_by_index(mid)
    if not deleted:
...
def export_messages():
    if not require_admin():
        return redirect(url_for("login"))
    # write messages CSV from DB and serve the file
    try:
        data.write_messages_csv(MESSAGES_CSV)
    except Exception as e:
        flash("Could not export messages.")
        return redirect(url_for("admin"))
